...
def construct_adj_cosine(adata, n_neighbors=6, use_rep='X_pca'):
    """Construct adjacency matrix based on cosine similarity"""
    sc.pp.pca(adata, n_comps=30,random_state=42)
    X = adata.obsm[use_rep] if use_rep in adata.obsm else adata.X.toarray()
    sim_matrix = cosine_similarity(X)
    n_spot = sim_matrix.shape[0]

    interaction = np.zeros([n_spot, n_spot])
    for i in range(n_spot):
        neighbors = sim_matrix[i].argsort()[::-1][1:n_neighbors + 1]  # descending
...
def construct_adj_correlation(adata, n_neighbors=6, use_rep='X_pca'):
    """Construct adjacency matrix based on Pearson correlation"""
    sc.pp.pca(adata, n_comps=30, random_state=42)
    X = adata.obsm[use_rep] if use_rep in adata.obsm else adata.X.toarray()
    n_spot = X.shape[0]
    ...
    interaction = np.zeros([n_spot, n_spot])
    for i in range(n_spot):
        neighbors = corr_matrix[i].argsort()[::-1][1:n_neighbors + 1]
        interaction[i, neighbors] = 1
...
def construct_adj_snn(adata, n_neighbors=6, use_rep='X_pca'):
    """Construct adjacency matrix based on Shared Nearest Neighbor (SNN)"""
    sc.pp.pca(adata, n_comps=30, random_state=42)
    X = adata.obsm[use_rep] if use_rep in adata.obsm else adata.X.toarray()
