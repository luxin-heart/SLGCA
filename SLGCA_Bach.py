...
class SLGCA():
    def __init__(self,
        ...
        self.adata_list = load_data(path=self.path, sample_list=sample_list, n_neighbors=self.n_neighbors,
                                    n_top_genes=self.n_top_genes, label=self.label)

        self.merged_adata = concatenate_slices(self.adata_list)

        self.adata = construct_whole_graph(adata_list=self.adata_list, merged_adata=self.merged_adata)
    ...
    def train(self):
        ...
        print("Training completed for Stereo ST data!")
        with torch.no_grad():
